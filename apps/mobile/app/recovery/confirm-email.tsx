...
export default function RecoveryConfirmEmail() {
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    try {
      await recoveryApi.confirmEmail(recoveryRequestId, code.trim());
      router.push("/recovery/confirm-phone");
    } catch (err) {
  ...
  return (
    <Screen>
      ...
      <TextField
        ...
        maxLength={6}
        testID="recovery-email-code-input"
      />

      {error ? <Text className="mb-4 text-sm text-danger">{error}</Text> : null}

      <Button
        testID="recovery-confirm-email-button"
        label="Confirm"
