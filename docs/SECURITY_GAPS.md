...
**Why:** Found while auditing `docs/SECURITY_GAPS.md`/`CLAUDE.md` for drift, not something anyone deliberately decided to leave gold. `docs/brand/generate_icon.py` (the earlier, fully programmatic "D" monogram the hand-supplied gold artwork superseded — see `docs/brand/README.md`'s "Earlier design" section) has had its constants corrected to the current `ui-tokens` palette in this pass, but was **not** run — swapping the shipped icon back to the simpler geometric monogram (losing the hand-supplied artwork's gradient/speech-bubble-tail design) is a real brand/visual call, not something to pick silently on the maintainer's behalf.

**Tracked for:** Either commission/produce a v2-palette version of the existing hand-supplied artwork (preserves the current design's actual look), or run the now-corrected `generate_icon.py` to fall back to the programmatic monogram in the current palette (simpler, immediately regenerable, but a different look) — a decision for whoever owns the visual brand next, not a code fix.
...
### Data export bundle's deeper joins are hand-picked, not a generic multi-hop walker (spec §34.4)

**What's missing:** `domain/compliance/export.py` (`DataExportService`) generates a real downloadable bundle when an admin completes an **access** request (`ComplianceService.complete` calls it automatically, uploads via `StorageProvider.put_object`, and stores the key on `DataSubjectRequest.export_storage_key`; `GET /account/data-requests/{id}/download` mints a fresh presigned URL). It's reflection-driven — walks every mapped model for a *direct* foreign key to `users.id` — plus three hand-written joins for tables one or two hops away that are unambiguously "this user's own data": `messages` (sender via `sender_device_id -> devices.user_id`), `media_objects` (one hop further, via `message_id -> messages`), and `location_pings` (via `location_share_id -> location_shares.sharer_user_id` — only the sharer's own pings, not a recipient's view of someone else's). Every column is filtered through the existing §5 classification registry: P0 excluded, P2 (ciphertext, precise location) redacted to a placeholder, P1/P3 included as-is. Tables reachable only through a relationship that isn't cleanly "this user's own data" — `meeting_participants`/`location_access_log`-adjacent rows for an interaction the user was merely part of rather than initiated, etc. — are still not walked.

**Why:** A fully generic multi-hop graph walker is a materially larger, separate piece of engineering than the specific joins built here, and risks silently traversing relationships that shouldn't be treated as "this user's own data" (e.g. every *other* participant's row in a meeting the user merely attended). Hand-picking the joins that matter — chosen as the largest safe, mechanically verifiable slice — avoids guessing at which deeper joins are "this user's data" vs. "data about an interaction the user was part of."

**Tracked for:** Add further deliberate, named joins only for tables that are similarly unambiguous, the same way `messages`/`media_objects`/`location_pings` were added here — never a generic walker. Not yet run against a real S3-compatible endpoint in this environment — same `SandboxStorageProvider`-unverified caveat as the "Local S3-compatible storage" entry above; `put_object` is new and shares that adapter's untested-against-a-live-bucket status.
