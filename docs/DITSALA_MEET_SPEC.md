...
**Meet Phase 3 (complete, with a disclosed scope cut):** transcripts, AI notes, in-meeting Q&A, and full-text search are all real — but built as a **post-meeting pipeline over the finished recording**, not the live per-track streaming pipeline §6 originally describes:

- **Why the cut**: live captions need a LiveKit Agents worker joining the room as a hidden participant, tapping each track in real time via the `livekit` core WebRTC client (`livekit-rtc`, native bindings — audio/video codecs, ICE, etc., not pure Python). This dev machine's resources at the time (1.2GB disk, 0.5GB RAM free) made installing that safely impossible, and writing integration code against an SDK never actually introspected here would break this project's own established discipline of verifying vendor SDK shapes before coding against them (see the `livekit-api` RoomService/EgressService verification earlier in Phase 2). A real, working, tested pipeline over the recording was judged better than an unverifiable, possibly-wrong one against the live-agents API — see `docs/SECURITY_GAPS.md`.
- **What's real**: `MeetingIntelligenceService.transcribe_recording` (`domain/meet_ai/service.py`) takes a `ready` `MeetingRecording`, gets a presigned download URL from the *existing* `StorageProvider` (§8 reuse), and sends that URL straight to Deepgram's real prerecorded `/v1/listen` REST endpoint (`services/meet_ai/deepgram.py`) with diarization on — DITSALA's backend never downloads or holds the recording bytes itself. Diarized word groups become `meeting_transcripts` rows, with Deepgram's numeric speaker index mapped to a `MeetingParticipant` by join order (a best-effort heuristic, not a guaranteed-correct identification, since there's no per-track tap tying a speaker index to a specific LiveKit identity in this pipeline).
- `generate_notes` sends the full transcript to Claude (`services/meet_ai/claude.py`, the `anthropic` SDK's Messages API, shape confirmed via live introspection of the installed package) and stores the structured result as `meeting_ai_notes` rows (`summary`/`decision`/`action_item`/`topic`), editable via `PATCH /meetings/{id}/notes/{note_id}` (§15's "allow users to edit").
- `ask` (`POST /meetings/{id}/ask`) answers a question against the transcript-so-far — the same "what did I miss" value §17 describes, just available once a transcript exists rather than continuously live.
- `GET /meetings/search?q=` (§18) does a real Postgres full-text search — generated, GIN-indexed `tsvector` columns on both `meetings.title` and `meeting_transcripts.text_segment` — scoped server-side to meetings the searching user actually participated in.
- Neither Deepgram nor Claude has a distinct sandbox mode (same reasoning as `RoomProvider`), and neither has been exercised against a live API key in this environment — see `docs/SECURITY_GAPS.md`, same caveat as Stitch/Smile ID/LiveKit.
- **Not built**: live in-meeting captions, and the LiveKit Agents worker itself.
