...
Not yet started: the rest of Phase 4 onward (Circle, location/SOS/calls, admin panel, recovery/hardening), plus the actual native module + chat UI whenever native build tooling is available. See "Execution order" in the spec — build in order, don't skip ahead.
